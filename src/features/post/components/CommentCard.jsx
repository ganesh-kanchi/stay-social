...
import { fetchPostDate } from "utilities";

export const CommentCard = ({ comment, postId }) => {
  const navigate = useNavigate();

  const { username, fullName, createdAt, text:commentText } = comment;
        ...
        <div className="flex justify-between ">
          ...
          >
            <div className="flex flex-col gap-0 2xl:flex-row 2xl:gap-1">
              {fullName && <span className="font-bold tracking-wide">{fullName}</span>}
              <span className="text-lightGrey">@{username}</span>
            </div>
            <span className="text-lightGrey">·</span>
            <div className="text-lightGrey">{fetchPostDate(createdAt)}</div>
          </div>
